...
const visibleProjects = realisations.filter(
    (project) => project.visible && !project.draft
);

const highlightedProject =
    visibleProjects.find((project) => project.highlight)
    ?? visibleProjects[0];

const otherProjects = visibleProjects.filter(
    (project) => project.slug !== highlightedProject.slug
);
...
const shuffledProjects = [...otherProjects].sort(
    (firstProject, secondProject) =>
        getStableHash(firstProject.slug, 17) -
        getStableHash(secondProject.slug, 17)
);
...
const revealOrderBySlug = new Map(
    [...shuffledProjects]
        .sort(
            (firstProject, secondProject) =>
                getStableHash(firstProject.slug, 91) -
                getStableHash(secondProject.slug, 91)
        )
...
const animationBySlug = new Map(
    [...shuffledProjects]
        .sort(
            (firstProject, secondProject) =>
                getStableHash(firstProject.slug, 53) -
                getStableHash(secondProject.slug, 53)
        )
        .map((project, index) => [
            project.slug,
...
export default function Realisations() {
    ...
    const btnStyle =
        "cta-button px-8 py-3 w-fit mx-auto";

    const btnStyle2 =
        "cta-button px-10 py-3";
