...
export async function POST(request: Request) {
    ...
    const contactFromEmail =
        process.env.CONTACT_FROM_EMAIL ?? "Arkanya <onboarding@resend.dev>";
    ...
    if (!resendApiKey || !contactToEmail) {
        ...
        return NextResponse.json(
            {message: "Configuration d'envoi manquante."},
            {status: 500}
        );
    }
    ...
    const resendResponse = await resend.emails.send(emailBody);
    ...
    if (error) {
        ...
        return NextResponse.json(
            {message: "L'envoi du message a echoue."},
            {status: 502}
        );
    }
