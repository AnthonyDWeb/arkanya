...
if (-not (Test-Path -LiteralPath $keystoreProperties)) {
  throw "android/keystore.properties est requis pour produire une release signee. Consultez README.md."
}
...
$publicApk = Join-Path $downloadDir "arkcare.apk"
$tempApk = Join-Path ([System.IO.Path]::GetTempPath()) "arkcare-release-previous-$([guid]::NewGuid()).apk"
$hadPreviousApk = Test-Path -LiteralPath $publicApk
if ($hadPreviousApk) {
  Move-Item -LiteralPath $publicApk -Destination $tempApk -Force
}

try {
  npm run build
  if ($LASTEXITCODE -ne 0) {
    throw "Le build Next a echoue avec le code $LASTEXITCODE."
  }
  npx cap sync android
  if ($LASTEXITCODE -ne 0) {
    throw "La synchronisation Capacitor a echoue avec le code $LASTEXITCODE."
  }
  Push-Location $androidDir
  try {
    .\gradlew.bat assembleRelease
    if ($LASTEXITCODE -ne 0) {
      throw "L'assemblage Gradle a echoue avec le code $LASTEXITCODE."
    }
  }
  finally {
    Pop-Location
  ...
  if (-not (Test-Path -LiteralPath $apkPath)) {
    throw "Gradle n'a pas produit l'APK de release attendu."
  }
