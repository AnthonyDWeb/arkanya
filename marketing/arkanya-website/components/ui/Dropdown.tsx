...
export default function Dropdown({ label, items }: DropdownProps) {
  ...
  return (
    ...
    >
      <div className="flex cursor-pointer items-center">{label}</div>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <div className="w-72 rounded-xl border border-[#ebe4ca]/12 bg-[#10141c]/94 p-2 shadow-[0_28px_90px_rgba(0,0,0,0.38)] backdrop-blur-xl">
              {items.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
