...
export default async function RealisationDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const aproachStyle =
    "absolute left-0 top-1 w-6 h-6 bg-surface border border-border-subtle rounded-full flex items-center justify-center text-xs font-medium";
  const metricStyle =
    "bg-surface rounded-xl p-8 border border-border-subtle shadow-soft h-full flex flex-col justify-center items-center text-center";
  const stackStyle =
    "px-5 py-2 text-sm font-medium rounded-full border border-gold/40 bg-gold/10 text-gold hover:bg-gold/20 transition";
      ...
      <section className="w-[90%] xl:w-[70%] mx-auto py-24 space-y-24">
        {project.context && (
          <section className="space-y-6">
            ...
            <ScrollReveal>
              <p className="text-text-medium leading-relaxed">{project.context}</p>
            </ScrollReveal>
          </section>
        )}
        {project.challenge && (
          <section className="space-y-6">
            <ScrollTitle>
              <h2 className="text-2xl font-semibold">Problématique stratégique</h2>
            </ScrollTitle>

            <ScrollReveal>
              <p className="text-text-medium leading-relaxed">{project.challenge}</p>
            </ScrollReveal>
          </section>
        )}
        {project.approach && (
        ...
        {project.metrics && project.metrics.length > 0 && (
          <section className="space-y-8">
            <ScrollTitle>
              <h2 className="text-2xl font-semibold">Indicateurs clés</h2>
            </ScrollTitle>
            ...
            {hasBefore ? (
              <>
                <div className="space-y-6">
                  <h3 className="text-lg font-medium text-text-medium">Avant</h3>
                ...
                {afterImages.length > 0 && (
                  <div className="space-y-6">
                    <h3 className="text-lg font-medium text-text-medium">Après</h3>
                    <div className="grid gap-8 md:grid-cols-2">
            ...
            <ScrollReveal>
              <div className="flex flex-wrap gap-3">
                {project.technologies.map((tech, index) => (
                  <span key={index} className={stackStyle}>
                    {tech}
                  </span>
                ))}
              </div>
